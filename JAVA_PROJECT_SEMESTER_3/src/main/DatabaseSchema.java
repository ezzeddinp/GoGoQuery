...
//    public static boolean decreaseQuantityFromCart()
    ...
    public static boolean updateItem(int itemId, String name, double price, String description, String category, int stock) {
        String query = "UPDATE msitem SET ItemName = ?, ItemPrice = ?, ItemDesc = ?, ItemCategory = ?, ItemStock = ? WHERE ItemID = ?";
        try (Connection connection = getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {
...
//    END MANAGER SECTION
    ...
    // Method untuk update package setelah manager klik send package
    ...
    public static List<Item> getAllItems() throws SQLException {
        ...
        try (Connection connection = getConnection();
            ...
            while (resultSet.next()) {
                int id = resultSet.getInt("ItemID"); // tidak boleh typo karena ini ngerefer ke atribut msitem
                ...
                int stock = resultSet.getInt("ItemStock");

                items.add(new Item(id, name, price, desc, category, stock));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public static int getItemStock(int itemId) throws SQLException {
        ...
        try (Connection connection = getConnection();
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                if (rs.next()) {
                    maxQuantity = rs.getInt("ItemStock");
                }
            }
        }
    ...
    public static void createTransactionDetails(int transactionId, List<Item> cartItems) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(query)) {
            for (Item item : cartItems) {
                ...
                stmt.setInt(3, item.getStock()); // Quantity
                stmt.addBatch();
            }
    ...
    public static class Item {
        ...
        public Item(int id, String name, double price, String description, String category, int stock) {
            this.id = id;
            this.name = name;
            this.price = price;
